...
def cisco_login(hostname, username, password, enable_password):
    ...
    try:
        ...
        if '#' in output or '>' in output:
            ...
            print("Login failed")
        channel.send("show interfaces status | include disabled\n")
        time.sleep(1)
        int_status_output = channel.recv(65535).decode('utf-8')
        print(int_status_output)
        lines = int_status_output.count('Gi1/0/')
        print("%d interfaces are in shutdown" %lines)
        ssh.close()
