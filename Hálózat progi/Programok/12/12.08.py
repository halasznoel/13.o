...
def cisco_login(hostname, username, password, enable_password):
    ...
    try:
        ...
        time.sleep(1)
        config_output = channel.recv(65535).decode('utf-8')
        print(config_output)
        if '(config)' in config_output:
            print("Entered configurate mode")
            for interface_number in range(10,15):
                channel.send("interface gi1/0/%s" % interface_number + "\n")
                channel.send("shutdown\n")
                channel.send("exit\n")
                print("interface gi1/0/%s" % interface_number + "was shut down\n")
                channel.send("end\n")
            print("Entered privilege mode")
        ssh.close()
